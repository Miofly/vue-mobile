...
MeScroll.prototype.setBounce = function (isBounce = true) {
    ...
    if (isBounce === false) {
        ...
        setTimeout(function () {
            const uniPageDom = document.getElementsByTagName('uni-page')[0]
            ...
            while (el !== document.body && el !== document) {
                ...
                if (cls) {
                    if (cls.contains('mescroll-touch')) { // 采用scroll-view 此处不能过滤mescroll-uni,否则下拉仍然有回弹
                        ...
                        // 如果配置了水平或者垂直滑动
                        const curX = e.touches ? e.touches[0].pageX : e.clientX // 当前第一个手指距离列表顶部的距离x
                        const curY = e.touches ? e.touches[0].pageY : e.clientY // 当前第一个手指距离列表顶部的距离y
                        if (!this.preWinX) this.preWinX = curX // 设置上次移动的距离x
                        if (!this.preWinY) this.preWinY = curY // 设置上次移动的距离y
                        // 计算两点之间的角度
                        const x = Math.abs(this.preWinX - curX)
                        const y = Math.abs(this.preWinY - curY)
                        const z = Math.sqrt(x * x + y * y)
                        this.preWinX = curX // 记录本次curX的值
                        this.preWinY = curY // 记录本次curY的值
                        if (z !== 0) {
                            const angle = Math.asin(y / z) / Math.PI * 180 // 角度区间 [0,90]
