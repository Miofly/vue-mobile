<template>
	<view class="u-form-item" :class="{'u-border-bottom': elBorderBottom, 'u-form-item__border-bottom--error': validateState === 'error' && showError('border-bottom')}">
		<view class="u-form-item__body" :style="{ flexDirection: elLabelPosition == 'left' ? 'row' : 'column'}">
			<!-- 微信小程序中，将一个参数设置空字符串，结果会变成字符串"true" -->
			<view class="u-form-item--left" :style="{ width: uLabelWidth, flex: `0 0 ${uLabelWidth}`, marginBottom: elLabelPosition == 'left' ? 0 : '10rpx'}">
				<!-- 为了块对齐 -->
				<view class="u-form-item--left__content" v-if="required || leftIcon || label">
					<!-- nvue不支持伪元素before -->
					<text v-if="required" class="u-form-item--left__content--required">*</text>
					<view class="u-form-item--left__content__icon margin-left-sm" v-if="leftIcon">
						<view :class="['fa-' + leftIcon]" :style="[leftIconStyle]" class="fa"></view>
					</view>
					<view class="u-form-item--left__content__label margin-left-sm" :style="[elLabelStyle, { 'justify-content': elLabelAlign == 'left' ? 'flex-start' : elLabelAlign == 'center' ? 'center' : 'flex-end' }]">
						{{label}}
					</view>
				</view>
			</view>
			<view class="u-form-item--right u-flex">
				<view class="u-form-item--right__content">
					...
					<view class="u-form-item--right__content__icon u-flex" v-if="$slots.right || rightIcon">
						<view :class="['fa-' + rightIcon]" v-if="rightIcon" :style="[rightIconStyle]" class="fa"></view>
						<slot name="right" />
					</view>
				</view>
			</view>
		</view>
		<view class="u-form-item__message" v-if="validateState === 'error' && showError('message')" :style="{paddingLeft: elLabelPosition == 'left' ? $mio.mioRoot.addUnit(elLabelWidth) : '0'}">{{validateMessage}}</view>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		uLabelWidth() {
			...
			return this.elLabelPosition == 'left' ? (this.label === 'true' || this.label === '' ? 'auto' : this.$mio.mioRoot.addUnit(this.elLabelWidth)) : '100%';
		},
		showError() {
			return type => {
	...
	methods: {
		broadcastInputError() {
			...
			this.broadcast('m-input', 'on-form-item-error', this.validateState === 'error' && this.showError('border'));
		},
		// 判断是否需要required校验
		setRules() {
			let that = this;
	...
	mounted() {
		...
		this.parent = this.$mio.mioRoot.$parent.call(this, 'm-form');
		if (this.parent) {
