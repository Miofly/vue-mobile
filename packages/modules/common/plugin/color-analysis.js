...
if (!pv) var pv = {
	map: function (a, b) {
		var c = {};
		return b ? a.map(function (a, d) {return c.index = d, b.call(c, a)}) : a.slice()
	}, naturalOrder: function (c, a) {return c < a ? -1 : c > a ? 1 : 0}, sum: function (a, b) {
		var c = {};
		return a.reduce(b ? function (a, e, d) {return c.index = d, a + b.call(c, e)} : function (a, b) {return a + b}, 0)
	}, max: function (a, b) {return Math.max.apply(null, b ? pv.map(a, b) : a)}
};
var MMCQ = function () {
	function a(a, c, d) {return (a << 10) + (c << 5) + d}

	function b(a) {
		function b() {c.sort(a), d = !0}

		var c = [], d = !1;
		return {
			push: function (a) {c.push(a), d = !1},
			peek: function (a) {return d || b(), void 0 === a && (a = c.length - 1), c[a]},
			pop: function () {return d || b(), c.pop()},
			size: function () {return c.length},
			map: function (a) {return c.map(a)},
			debug: function () {return d || b(), c}
		}
	}
	...
	function e(b) {
		...
		return b.forEach(function (b) {d = b[0] >> 3, e = b[1] >> 3, f = b[2] >> 3, c = a(d, e, f), g[c] = (g[c] || 0) + 1}), g
	}

	function f(a, b) {
		var d, e, f, g = 1e6, h = 0, i = 1e6, j = 0, k = 1e6, l = 0;
		return a.forEach(function (a) {d = a[0] >> 3, e = a[1] >> 3, f = a[2] >> 3, d < g ? g = d : d > h && (h = d), e < i ? i = e : e > j && (j = e), f < k ? k = f : f > l && (l = f)}), new c(g, h, i, j, k, l, b)
	}
	...
	return c.prototype = {
		...
		nearest: function (a) {
			for (var b, c, d, e = Math.pow, f = this.vboxes, g = 0; g < f.size(); g++) c = Math.sqrt(e(a[0] - f.peek(g).color[0], 2) + e(a[1] - f.peek(g).color[1], 2) + e(a[2] - f.peek(g).color[2], 2)), (c < b || void 0 === b) && (b = c, d = f.peek(g).color);
			return d
		},
		forcebw: function () {
			var a = this.vboxes;
			a.sort(function (c, a) {return pv.naturalOrder(pv.sum(c.color), pv.sum(a.color))});
			var b = a[0].color;
		...
		quantize: function (a, c) {
			...
			var i = e(a), j = 0;
			i.forEach(function () {j++}), j <= c;
			var k = f(a, i), l = new b(function (c, a) {return pv.naturalOrder(c.count(), a.count())});
			l.push(k), h(l, .75 * c);
			for (var m = new b(function (c, a) {return pv.naturalOrder(c.count() * c.volume(), a.count() * a.volume())}); l.size();) m.push(l.pop());
			h(m, c - m.size());
	...
	function a(b) {_classCallCheck(this, a), this.canvasId = b}

	return _createClass(a, [{
		key: "getPalette",
		value: function (a, c) {
			var b = this, d = a.width, e = a.height, f = a.imgPath, h = a.maxColors, j = a.step;
			...
			g.drawImage(f, 0, 0, d, e), g.draw(!1, function () {
				uni.canvasGetImageData({
					...
					height: e,
					success: function (f) {
						for (var k, l, m, n, o, p = f.data, q = [], s = 0; s < d * e; s += j) k = 4 * s, l = p[k + 0], m = p[k + 1], n = p[k + 2], o = p[k + 3], 125 <= o && !(250 < l && 250 < m && 250 < n) && q.push([l, m, n]);
						var t = MMCQ.quantize(q, h), u = t ? t.palette() : null;
						c && c(u)
					}
				})
			})
