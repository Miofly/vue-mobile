...
export default class index extends Vue {
    ...
    onLoad () {
        lottieInstance = wx.createSelectorQuery().select('#canvas').node(res => {
            ...
            lottie.setup(canvas)
            lottieInstance = lottie.loadAnimation({ // 微信小程序给的接口，调用就完事了，原理不太懂a
                loop: true, // 是否循环播放（选填）
                autoplay: true, // 是否自动播放（选填）
                path: 'https://assets10.lottiefiles.com/packages/lf20_el2hc9j4.json', // lottie json包的网络链接，可以防止小程序的体积过大，要注意请求域名要添加到小程序的合法域名中
