		...
		<view :class="[(fadein || show)?'mio-modal-normal':'mio-modal-scale',show?'mio-modal-show':'']"
			...
			<view>
				<view :style="{fontSize: closeSize + 'rpx'}" @tap="show = false" class="fa fa-times-circle-o"
					  ...
					  v-if="closeShow"></view>
				<view :style="{fontSize: titleSize + 'rpx', color: titleColor}" class="mio-modal-title" v-if="title">
					{{title}}
				</view>
				<view :style="{color:descColor,fontSize:descSize+'rpx'}"
