<template>
	<view>
		<Chart :options="chartOptions" :style="{width: width, height: height}"></Chart>
	</view>
</template>
...
<script lang="ts">
    ...
    export default class chartIndex extends Vue {
        @Prop({ default: 'transparent', type: String }) backgroundColor?: string //
        @Prop({ default: '2300rpx', type: String }) width?: string //
        @Prop({ default: '400rpx', type: String }) height?: string //
        @Prop({ default () { return ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月', '一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'] }, type: Array }) xData?: any []
		@Prop({ default () { return [3.9, 4.2, 5.7, 8.5, 11.9, 15.2, 17.0, 16.6, 14.2, 10.3, 6.6, 4.8, 7.0, 6.9, 9.5, 14.5, 18.4, 21.5, 25.2, 26.5, 23.3, 18.3, 13.9, 9.6] }, type: Array }) yData?: any []
        get chartOptions () {
            return {
                chart: {
                ...
                xAxis: {
                    ...
                    labels: { // 坐标轴文字
                        ...
                        // formatter () { // 对坐标轴文字的处理
                        //     const labelVal = this.value // 获取到刻度值
                        //     const reallyVal = labelVal // 实际返回的刻度值
                        //     return reallyVal
                        // }
                    },
                ...
                tooltip: {
                    ...
                    useHTML: true,
                    formatter () { // 自定义提示处理
                        return (`${this.x}<br>` + `<span style="font-weight: bold">下载量</span>：${this.y}`) // eslint-disable-line
                    }
                },
                plotOptions: {
                    areaspline: {
                ...
                series: [
                    {
                        ...
                        dashStyle: [
                            'Dot', 'Solid', 'ShortDash', 'ShortDot', 'ShortDashDot',
                            'ShortDashDotDot', 'Dot', 'Dash', 'LongDash', 'DashDot',
                            'LongDashDot', 'LongDashDotDo'
                        ][0]
                    }
                ],
                legend: { // 图列样式设置
