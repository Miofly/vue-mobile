<template>
	...
	-->
	<mescroll-uni ref="mescrollRef" height="80%" top="0" bottom="100" :down="downOption"
		...
		<view class="cu-list menu" :class="[false?'sm-border':'', false?'card-menu margin-top':'']">
			<view v-for="(item, index) in dataLists" :key="index" class="cu-item">
				<view class="content flex justify-between align-center">
					...
					<view class="width-fifty-five fr text-red ">{{tu.getLocalData('isMoney') == 1 ? '总收入' : '总计费次数'}}：{{item.effective_click_total}}</view>
				</view>
