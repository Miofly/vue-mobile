<template>
	<view class="full-width-height">
		...
		z-index: 9999;padding-top: 80rpx">
			...
			<view v-if="is_department_captain || is_team_captain" class="text-right">
				<button @tap="jumpPage" class="cu-btn margin-right margin-top-ten-lower"
						:class="[['bg-white', 'line-white', 'line-white lines-white'][0],
		...
		<scrollMess :text="mess" style="position: fixed;top: 240rpx;left: 0;z-index: 9999;height: 60rpx"
					:speed="60" background-color="#fffbe8" color="#de8c17"
					:showClose="false" showIcon="true" :scrollable="true" :single="true"
					:showGetMore="false" :moreText="false ? '查看更多' : ''" moreColor="#999999" @getmore="getMore">
		</scrollMess>
		...
		<mescroll-uni ref="mescrollRef" height="100vh" top="300" bottom="80" :down="downOption"
					...
					<view class="content padding-tb-sm" style="overflow-x: hidden!important;">
						<view class="padding-top-bottom padding-right-xl" style="width: 100%;">
							...
							<view class="padding-left-xl " style="width: 75%;height: auto;overflow-x: hidden">
								...
								<view style="overflow: hidden" class="text-df text-grey">
									{{ item.desc }}
								</view>
								<view class="flex">
									<button class="cu-btn fl" style="width: 150rpx" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
								        ['sm', 'lg', ''][0], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
											@tap="getLink(item.id)">
										<text v-show="true" class="fa fa-wechat" style="padding-right: 10rpx"></text>
										<text class="text-df" style="width: 100rpx">链接</text>
									</button>
									<button class="cu-btn fr" style="margin-left: 10px;width: 150rpx" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
								        ['sm', 'lg', ''][0], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
											@tap="jumpWx(item.title, item.desc, item.pic, item.id)">
										<text v-show="true" class="fa fa-wechat" style="padding-right: 10rpx"></text>
										<text class="text-df" style="width: 100rpx">图文</text>
									</button>
								</view>
							</view>
						</view>
		...
		<view @tap="getShortChain" style="position: fixed;bottom: 12%;z-index: 999999999;"
			  class="text-center margin-center full-width">
		...
		<modal title="标题" content="这是内容" :show="modalStatus"
			...
			<view class="fa fa-close" style="position: absolute; top:20px;right: 20px;font-size: 1.2rem"
				  @tap="closeModalTwo"></view>
			...
			<view class="text-center padding" style="border: 1px solid rgba(0, 0, 0, 0.7);border-radius: 10px">
				...
				<view v-else v-for="(url_list, index) in url_lists" :key="index">
			...
			<view class="flex justify-around margin-top">
				<button @tap="replace" class="cu-btn" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
				        ['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']">
					<text v-show="false" class="fa fa-wechat padding-right-twenty"></text>
					换一批
				</button>
				<button @tap="modalWx" class="cu-btn" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
				        ['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']">
					<text v-show="false" class="fa fa-wechat padding-right-twenty"></text>
					复制并打开微信
				</button>
		...
		<modal title="标题" content="这是内容" :show="modalStatusTwo"
			   :custom="true">
			<view class="fa fa-close" style="position: absolute; top:10px;right: 10px;font-size: 1.2rem"
				  @tap="closeModal"></view>
			<view class="text-center padding" style="border: 1px solid rgba(0, 0, 0, 0.7);border-radius: 10px">
			...
			<view class="flex justify-around margin-top">
				<button @tap="modalWxTwo" class="cu-btn" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
				        ['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']">
					<text v-show="false" class="fa fa-wechat padding-right-twenty"></text>
					复制并打开微信
				</button>
				<button @tap="copyTwo" class="cu-btn" :class="[['bg-red', 'line-red', 'line-red lines-red'][0],
				        ['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']">
					<text v-show="false" class="fa fa-wechat padding-right-twenty"></text>
					复制链接
				</button>
    ...
    export default {
        ...
        methods: {
            ...
            async replace() {
                this.tempVar = true
                this.url_lists = []
                const data = await commonPost('/title/title-share-batch', {url_type: 0, trans_product_id: this.radioVal})
            ...
            async getLink(id) {
                ...
                const data = await commonPost('/title/title-share', {id: id, url_type: 0, trans_product_id: this.radioVal})
            ...
            async jumpWx(title, desc, pic, id) {
                ...
                if (data.data == null) {
                    ...
                    if (navigator.userAgent.toLowerCase().indexOf('micromessenger') !== -1) {
                        this.ui.showToast('图文分享请打开QQ浏览器，右上角选择QQ浏览器')
                        // location.href = 'mqqbrowser://url=http://web.lezhuan2020.cn'
                    } else {
                        try {
