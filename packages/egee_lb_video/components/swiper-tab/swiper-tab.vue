		...
		<view class="longTab">
			<scroll-view scroll-x="true" style="white-space: nowrap; display: flex" scroll-with-animation
				...
				<view class="underlineBox"
					...
					<view class="underline" :style="{background: lineColor, width: underLineWidth}"
...
<script>
    ...
    export default {
        ...
        created() {
            this.myTab = this.$store.state.nowIndex
            ...
            uni.getSystemInfo({ // 用于控制每个tab的宽度
                success(e) {
                    if (that.tabLists.length <= 5) {
						...
						that.isLeft = that.$store.state.nowIndex * that.isWidth
                    } else {
