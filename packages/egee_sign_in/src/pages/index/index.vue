...
export default class index extends Vue {
    ...
    // 正常签到
    async signInFn (duplicated?) {
        const { data, code } = await commonPost(`/checkin/done.html?uuid=${this.$store.state.center.uuid}`, { option: 1, duplicated: 0 }, { 'content-type': 'application/x-www-form-urlencoded' })
        if (code == 200) {
            this.modalStatus = false
            this.getData()
            this.$mio.mioRoot.showToast('签到成功')
        }
    }
    ...
    async signInPatchFn () {
        ...
        if (code == 200) {
            ...
            this.getData()
            this.$mio.mioRoot.showToast('补签成功，可继续签到')
        }
    }
