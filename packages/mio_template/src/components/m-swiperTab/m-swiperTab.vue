<template>
	<view :style="{width: fullWidth, color: defaultColor}" style="position: relative" class="bg-white">
		<view class="full-width">
			<scroll-view :scroll-left="tabLeft" scroll-with-animation scroll-x="true" style="white-space: nowrap; display: flex;">
				<view :data-index="index" :id="'id'+index" :key="index" :style="{color: index===myTab ? activeColor : '', width: isWidth + 'px', fontSize: index===myTab ? activeSize : textSize,
					...
					<view style="position: relative">
						...
						<view v-if="tagStatus && item.num != 0" class="cu-tag badge"
							  :style="{top: tagTop + 'rpx', right: tagRight + 'rpx', background: tagBg}">{{item.num}}</view>
					</view>
				</view>
				<!--下划线-->
