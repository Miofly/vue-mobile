<template>
	<view :style="[itemStyle]" class="u-collapse-item">
		<view :hover-class="hoverClass" :hover-stay-time="200" :style="[headStyle]" @tap.stop="headClick"
			  class="u-collapse-head">
			<block v-if="!$slots['title-all']">
				<view :style="[{ textAlign: align ? align : 'left' },
					isShow && activeStyle && !arrow ? activeStyle : '']" class="u-collapse-title u-line-1" v-if="!$slots['title']">
					{{ title }}
				</view>
				<slot name="title" v-else/>
				<view class="u-icon-wrap">
					<u-icon :class="{ 'u-arrow-down-icon-active': isShow }" :color="arrowColor" class="u-arrow-down-icon"
							name="arrow-down" v-if="arrow"></u-icon>
				</view>
			</block>
			<slot name="title-all" v-else/>
		</view>
		<view :style="[{
