			...
			<view class="mescroll-uni-content" :style="{'transform': translateY, 'transition': transition}">
				...
				<view v-if="mescroll.optDown.use" class="mescroll-downwarp"
					...
					<view class="downwarp-content">
						...
						<view class="downwarp-tip" :style="{fontSize: mescroll.optDown.downSize}">{{ downText }}</view>
				...
				<view v-if="mescroll.optUp.use && !isDownLoading" class="mescroll-upwarp"
					...
					<view v-show="upLoadType===1">
						...
						<view class="upwarp-tip" :style="{fontSize: mescroll.optUp.optSize}">
					...
					<!-- 无数据 -->
					<view v-if="upLoadType===2" class="upwarp-nodata" :style="{fontSize: mescroll.optUp.optSize}">{{
    ...
    export default {
        ...
        computed: {
            ...
            translateY() {
                ...
                return !this.down.use ? '' : this.downHight > 0 ? 'translateY(' + this.downHight + 'px)' : '' // transform会使fixed失效,需注意把fixed元素写在mescroll之外
            },
            // 是否在加载中
            isDownLoading() {
        ...
        created() {
            ...
            const myOption = JSON.parse(JSON.stringify({
                down: vm.down,
                up: vm.up
            })) // 深拷贝,避免对props的影响
            ...
            setClientHeight() {
                if (this.mescroll.getClientHeight(true) === 0 && !this.isExec) {
                    ...
                    this.$nextTick(() => { // 确保dom已渲染
                        const view = uni.createSelectorQuery().in(this).select('#' + this.viewId)
