<template>
	<view class="full-width-height" style="color: #000!important;">
		<scrollMess :moreText="false ? '查看更多' : ''" :scrollable="true"
					...
					style="position: fixed;top: 0rpx;left: 0;z-index: 9999;height: 60rpx">
		</scrollMess>
		<view class="full-width text-bg-new flex align-center" style="background: #fbcf24;height: 180rpx;position: fixed;top: 60rpx;left: 0;
		z-index: 9999;">
			<view class="flex justify-between width-sixty text-center text-lg margin-left"
				  style="border-radius: 0.3rem;border: 2px solid white;">
				<view :class="[status? ' bg-white new_active' : 'text-black']" @tap="teamSwitch" class="padding-xl"
					  style="width: 50%;border-bottom-right-radius: 0;
				;border-top-right-radius: 0;font-size: 14px" vs>团队标题
				</view>
				<view :class="[!status? 'bg-white new_active' : 'text-red']" @tap="personSwitch" class="padding-xl text-black"
					  style="width: 50%;border-bottom-left-radius: 0;
				;border-top-left-radius: 0;font-size: 14px">公用标题
				</view>
			</view>

			<view class="text-right" style="position: fixed; top: 115rpx;right: 20rpx;z-index: 999999;width: 250rpx;"
				  v-if="is_department_captain || is_team_captain">
				<button style="background: #ef560e;color: white" :class="[['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
						@tap="jumpPage"
		...
		<!--<view style="position: fixed;top: 300rpx;background: white;height: 100rpx;text-align: center;left: 0;width: 100%;z-index:999;">-->
			...
			<!--<radio-group @change="radioChange" style="text-align: center;display: inline-block;margin-top: 10rpx">-->
				<!--<label :key="item.value" style="display: inline-block;margin-left: 30rpx"-->
					   <!--v-for="(item, index) in items">-->
					<!--<radio :checked="index === current" :value="item.value"/>-->
					<!--<view>{{item.name}}</view>-->
				<!--</label>-->
			<!--</radio-group>-->
		<!--</view>-->
		...
		<mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @emptyclick="emptyClick"
					  ...
					  height="100vh" ref="mescrollRef" top="240" style="margin-bottom: 80px">
			<!--数据列表-->
			<view :class="[true?'sm-border':'', true?'card-menu margin-top':'']" class="cu-list menu"
				  style="overflow-x: hidden!important;width: 100vw;padding-left: 0!important;
				  padding-right: 0!important;margin-left: 0!important;margin-right: 0!important;border-radius: 0!important;">
				<view :key="index" class="cu-item" v-for="(item, index) in dataLists" style="margin-bottom: 20rpx;border: 0px solid transparent!important">
					<view class="content padding-tb-sm padding-bottom" style="overflow-x: hidden!important;">
						<view class="padding-top-bottom padding-right-xl" style="width: 100%;">
							...
							<image :src="item.pic"
								   style="height: 180rpx;width: 200rpx"
							></image>
						...
						<view class="flex justify-around">
							<button :class="['sm',false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
									@tap="getLink(item.id)" class="cu-btn fl"
									style="width: 150rpx;background: #ef560e;color: white">
								<!--<text class="fa fa-wechat" style="padding-right: 10rpx" v-show="true"></text>-->
								<text class="text-df" style="width: 100rpx">链接</text>
							</button>
							<button :class="['sm', false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
									@tap="jumpWxQq(item.title, item.desc, item.pic, item.id)" class="cu-btn fr"
									style="width: 200rpx;background: #ef560e;color: white">
								<!--<text class="fa fa-wechat" style="padding-right: 10rpx" v-show="true"></text>-->
								<text class="text-df">qq图文</text>
							</button>
							<button :class="['sm',false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
									@tap="jumpWx(item.title, item.desc, item.pic, item.id)" class="cu-btn fl"
									style="width: 260rpx;background: #ef560e;color: white">
								<!--<text class="fa fa-wechat" style="padding-right: 10rpx" v-show="true"></text>-->
								<text class="text-df" style="">百度图文</text>
							</button>
						</view>
					</view>
		...
		<view @tap="getShortChain" class="text-center margin-center full-width"
			...
			<button style="background: rgb(254, 215, 50);box-shadow: 0 0 8px rgba(26, 26, 26, 0.4);width: 50vw;height: 100rpx;font-weight: bold" :class="[['sm', 'lg', ''][1], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
					class="cu-btn">
				<text :disabled="false" class="fa fa-wechat padding-right-twenty" v-show="false"></text>
				批量获取链接
			</button>
		</view>
		<modal :custom="true" :show="modalStatus"
			...
			<view class="flex justify-around margin-top">
				...
				<!--<button @tap="copyVal" class="cu-btn" :class="['text-btn-new',-->
				<!--['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']">-->
		...
		<modal :custom="true" :show="modalStatusTwo" content="这是内容"
			...
			<view class="text-center padding" style="border: 1px solid rgba(0, 0, 0, 0.7);border-radius: 10px;">
				...
				<!--<view style="" v-else>{{shortChain}}</view>-->
			</view>
			<view class="flex justify-around margin-top">
				<button :class="['text-btn-new',
				        ['sm', 'lg', ''][2], false ? 'round' : '', true ? 'shadow' : '', false ? 'block' : '']"
        ...
        methods: {
            ...
            async replace() {
                ...
                const data = await commonPost('/title/title-share-batch', {
                    ...
                    group_code: this.radioVal == 37 ? 1006 : ''
                })
            ...
            async jumpWxQq(title, desc, pic, id) {
                ...
                if (data.data == null) {
                    ...
                    if (navigator.userAgent.toLowerCase().indexOf('micromessenger') !== -1) {
                        this.ui.showToast('图文分享请打开百度APP或者qq浏览器')
            ...
            async jumpWx(title, desc, pic, id) {
                this.tu.getClipboardData('http://web.xiyouzhuan2020.com')
                this.ui.showLoading()
                const data = await commonPost('/title/title-share', {
                    id: id,
                    url_type: 0,
