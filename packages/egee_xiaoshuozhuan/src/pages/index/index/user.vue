<template>
	<view class="full-width-height">
		<view class="text-bg-new padding text-center margin-center">
			<view class="cu-avatar margin-bottom-sm" :class="[false ? 'radius' : 'round', ['sm', 'lg', 'xl', ''][2], ]" style="background-image:url(/static/images/lz/xiyou.jpg);">
			<!--<view class="cu-avatar margin-bottom-sm" :class="[false ? 'radius' : 'round', ['sm', 'lg', 'xl', ''][2], ]" style="background-image:url(http://e-static.oss-cn-shanghai.aliyuncs.com/img/wfd/wz.png);">-->
			<!--<view class="cu-avatar margin-bottom-sm" :class="[false ? 'radius' : 'round', ['sm', 'lg', 'xl', ''][2], ]" style="background-image:url(http://e-static.oss-cn-shanghai.aliyuncs.com/img/wfd/wfx.png);">-->
		...
		<mescroll-uni ref="mescrollRef" height="100vh" bottom="0"
			...
			<view class="cu-list grid" :class="['col-' + 3, true?'':'no-border']">
				<view class="cu-item" v-for="(item, index) in jfLists" :key="index">
					...
					<view class="text-bold text-xyz text-xxl">{{item.frequency}}</view>
					<text>{{item.name}}</text>
				</view>
			</view>
...
<script>
    ...
    export default {
		...
		computed: {
            jfLists: function () {
                return [
                    {frequency: this.todayClick, color: 'xyz', badge: 0, name: '今日计费次数'},
                    {frequency: this.yesterdayClick, color: 'xyz', badge: 0, name: '昨日计费次数'},
                    {frequency: this.monthClick, color: 'xyz', badge: 0, name: '本月计费次数'}
                ]
            }
		},
		methods: {
