<template>
	<view class="full-height">
		...
		<search class="test" @question="question" :hotStatus="true"
            ...
            border-top-right-radius: 0;" class="cu-item share">
				...
				<ad v-if="ggkz" unit-id="adunit-0401c255cc4b89d3" ad-type="video" style="transform: scale(0.6)" ad-theme="black"></ad>
			</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async initData (val) {
                const INVOKE_CLOUD_FUNCTION_URL = `/sjh/mobile/ajax.html?mobile=${val}&operate=query`
                const data = await commonGet(INVOKE_CLOUD_FUNCTION_URL)
                this.phoneNum = val
                if (data.status) {
