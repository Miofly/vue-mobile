        ...
        <modal :closeShow="true" :closeSize="40" :descSize="30" :maskClosable="true" :status.sync="modalStatus"
            ...
            <view class="text-center margin-top-xxl">
                ...
                <view class="flex justify-around margin-top">
                    <m-button :customStyle="{fontSize: '14px', background: 'linear-gradient(180deg, #69ABFE 0%, #3173FE 100%)'}" :disabled="false" :hairLine="true" :loading="false" :plain="false"
...
export default class mescrollSwiper extends mixins(scrollMixins) {
    ...
    async jumpWxQq (title, desc, pic, id) {
        ...
        if (data.data === null) {
            ...
            location.href = 'mqqbrowser://url=http://web.hxz2020.com'
        } else {
            try {
    ...
    async jumpWx (title, desc, pic, id) {
        this.$mio.mioRoot.copyText('http://web.hxz2020.com')
        this.$mio.mioRoot.showLoading()
        const data = await appletsPost('title/title-share', {
            id,
            url_type: 0,
        ...
        if (data.data === null) {
            ...
            this.$mio.mioRoot.showToast('图文分享请打开百度APP或者qq浏览器')
            location.href = 'baiduboxapp://url=http://web.hxz2020.com'
        } else {
            try {
                if (url.charAt(url.length - 1) == '?') {
                    url = url.substring(0, url.length - 1)
    ...
    async upCallback (page) {
        ...
        const data = await appletsPost('title/title-list', { common: 1, page: pageNum, per_page: pageSize }) // 默认数据
