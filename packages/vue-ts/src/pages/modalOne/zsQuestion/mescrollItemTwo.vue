			...
			<view :key="index" style="display: flex;flex-direction: row;align-items: flex-start;"
				...
				<view class="u-column">
					<view class="demo-warter" style="position: relative">
						...
						<view class="bg-loveFive radius" style="height: 3px"></view>
...
<script lang="ts">
    ...
    export default class mescrollSwiper extends mixins(scrollMixins) {
        ...
        async upCallback (page) {
            ...
            if (this.nowType == 'zdy') {
                const newSize = 30
                const curPageData = page.num == 1 ? this.answerQuestion.slice(0, newSize) : this.answerQuestion.slice((page.num - 1) * newSize, page.num * newSize)
                const curPageLen = curPageData.length // 当前页数据长度
                const totalPage = Math.ceil(this.answerQuestion.length / newSize)
