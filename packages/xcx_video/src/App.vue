<script>
    ...
    export default {
        async onLaunch () {
            ...
            const data = await publicPost('xcx/conf')

            const res2 = await publicGet('classify/classify-list')

            // const data = await publicGet('classify/classify-list')
			this.$store.state.shLists = data.data
			this.$store.state.tabLists = res2.data
        },
        onShow () {
            // #ifdef MP-WEIXIN
            // this.share()
        ...
        computed: {
            ...mapState(['indexData', 'indexArr', 'defaultPage', 'masters', 'tabLists', 'shLists']),
        },
    }
</script>
